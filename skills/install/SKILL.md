...
Also tell the user they can run `/glm-statusline:configure` to choose which fields are shown; that command opens an interactive selector and previews after each selection. Press `l` in that selector to switch between the single-line and grouped layouts.
